...
let rec trim cs =
  ...
  let parse (s : string) : expr option =
    let rec parse_expr tokens =
      match tokens with
      | [] -> None
      | '(' :: 'a' :: 'd' :: 'd' :: rest when String.trim (String.concat "" (List.map Char.escaped (List.rev rest))) = "add" -> parse_add rest
      | '(' :: 'm' :: 'u' :: 'l' :: rest when String.trim (String.concat "" (List.map Char.escaped (List.rev rest))) = "mul" -> parse_mul rest
      | _ -> parse_num tokens
