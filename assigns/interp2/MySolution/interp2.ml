...
let parse_int =
  ...
  let is_alphanumeric c = (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9')

  let implode ls =
    String.of_seq (List.to_seq ls)
    
  let parse_symbol =
      let* first_char = satisfy is_lower_alpha in
      let* rest_chars = many (satisfy is_alphanumeric) in
      let char_list = first_char :: rest_chars in
      let symbol_str = implode char_list in
...
let rec parse_com() = 
  ...
  keyword "Else" >>
  let* false_branch = parse_coms () in
  keyword "End" >>
...
| [] -> None
| (x1,c1) :: v -> (
  if x1=x then Some (c1) else lookup_variable (x)(v)
)

let rec eval (s : stack) (t : trace) (p : prog) (v: env ) : trace =
  match p with
  (* termination state returns the trace *)
  | [] -> t
  | Push c :: p0 (* PushStack *) -> eval (c :: s) t p0 v 
  | Pop :: p0 ->
    (match s with
     | _ :: s0 (* PopStack *) -> eval s0 t p0 v
     | []      (* PopError *) -> eval [] ("Panic" :: t) [] [])
  | Trace :: p0 ->
    (match s with
     | c :: s0 (* TraceStack *) -> eval (Unit :: s0) (toString c :: t) p0 v
     | []      (* TraceError *) -> eval [] ("Panic" :: t) [] [])
  | Add :: p0 ->
    (match s with
     | Int i :: Int j :: s0 (* AddStack *)  -> eval (Int (i + j) :: s0) t p0 v
     | _ :: _ :: s0         (* AddError1 *) -> eval [] ("Panic" :: t) [] []
     | []                   (* AddError2 *) -> eval [] ("Panic" :: t) [] []
     | _ :: []              (* AddError3 *) -> eval [] ("Panic" :: t) [] [])
  | Sub :: p0 ->
    (match s with
     | Int i :: Int j :: s0 (* SubStack *)  -> eval (Int (i - j) :: s0) t p0 v
     | _ :: _ :: s0         (* SubError1 *) -> eval [] ("Panic" :: t) [] []
     | []                   (* SubError2 *) -> eval [] ("Panic" :: t) [] []
     | _ :: []              (* SubError3 *) -> eval [] ("Panic" :: t) [] [])
  | Mul :: p0 ->
    (match s with
     | Int i :: Int j :: s0 (* MulStack *)  -> eval (Int (i * j) :: s0) t p0 v
...
| If (true_branch, false_branch) :: p0 ->
    (match s with
     | Bool true_val :: s0 -> 
        let execute_this = if true_val= true then true_branch else false_branch in
        eval s0 t (execute_this @ p0 ) v 
     | _ ->
      eval [] ("Panic" :: t) [] [])
| Bind :: p0 -> 
  (match s with 
...
| Lookup :: p0 ->
  (match s with
  | Symbol x :: tl ->
    (match lookup_variable x v with
    | Some const -> eval (const :: tl) t p0 v
    | None -> eval [] ("Panic" :: t) [] [])
  | _ -> eval [] ("Panic" :: t) [] [])
