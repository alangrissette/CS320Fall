...
let rec mylist_get_at (xs: 'a mylist) (i0: int): 'a =
  match (xs, i0) with
  | (Nil, _) -> mylist_subscript_exn
  | (_, _) when i0 < 0 -> mylist_subscript_exn
  | (Cons(x, _), 0) -> xlet
  | (Cons(_, rest), i) when i > 0 -> mylist_get_at rest (i - 1)
...
let string_sepjoin_list (sep: string) (xs: string list): string =
  let rec concat (sep: string) (xs: string list) =
    match xs with
    | [] -> MyNil
    | x1::x2 -> 
      let combined = combine_mylists (MyCons (x1, MyNil)) sep in
      combine_mylists combined (concat sep x2)
  in
  let combine_mylists lst1 lst2 =
    MyCons (lst1, lst2)
  in
  list_concat (concat sep xs)
;;
